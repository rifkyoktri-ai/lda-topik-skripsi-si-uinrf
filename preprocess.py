...
DOMAIN_STOPWORDS_ADDITIONAL = {
    'analisis', 'analis', 'evaluasi', 'implementasi', 'pengembangan',
    'kembang', 'penerapan', 'terap', 'pembangunan', 'bangun', 'perancangan',
    'rancang', 'pembuatan', 'buat', 'studi', 'literatur', 'pustaka', 'kajian',
    'pendekatan', 'masalah', 'kendala', 'sulit', 'harap', 'manfaat', 'penting',
    'jumlah', 'hitung', 'ukur', 'sesuai', 'pilih', 'temu', 'laksana', 'upa',
    'sebab', 'cepat', 'manual', 'fitur', 'akses', 'dimensi', 'aju',
    'kata_kunci', 'keywords', 'keyword', 'kata',
    'sma_bina_warga', 'smk_muhammadiyah', 'pondok_pesantren',
    'inlislite', 'livin_mandiri', 'maxim', 'sjakhyakirti', 'mohon'
}
...
POST_STEM_STOPWORDS = {
    ...
    'nkata', 'kunci', 'nkata_kunci', 'kata_kunci', 'keywords', 'keyword', 'kata',
    'mana', 'namun', 'utama', 'dekat', 'kota',
...
def bersihkan(teks: str) -> str:
    """Pembersihan teks dasar (newline, lowercasing, dan karakter non-huruf)."""
...
def run_preprocessing(use_tfidf: bool = False, min_ngram_count: int = 2):
    ...
    logger.info(f"  Total Dokumen mentah: {len(df)}")
    
    # Filter duplikat berdasarkan Judul (case-insensitive, keep='first')
    df['temp_title_lower'] = df['title'].astype(str).str.strip().str.lower()
    len_before_dedup = len(df)
    df = df.drop_duplicates(subset=['temp_title_lower'], keep='first').drop(columns=['temp_title_lower']).reset_index(drop=True)
    logger.info(f"  Total Dokumen setelah hapus duplikat judul: {len(df)} (dibuang: {len_before_dedup - len(df)})")
    ...
    # Filter out documents with token count < 5
    len_before_len_filter = len(df)
    df = df[df['tokens'].apply(len) >= 5].reset_index(drop=True)
    logger.info(f"  Total Dokumen setelah filter token < 5: {len(df)} (dibuang: {len_before_len_filter - len(df)})")
